...
import { recordStateChange, recordAssignmentChange, recordPriorityChange } from '@/actions/record-history';
...
export async function takeTicket(ticketId: string) {
  ...
  try {
    ...
    const currentTicket = await pb.collection('tickets').getOne(ticketId);
    ...
    await recordAssignmentChange(ticketId, null, user.id);
    // Record initial state change
    if (currentTicket.estado !== 'en_proceso') {
      await recordStateChange(ticketId, currentTicket.estado, 'en_proceso');
    }
...
export async function updateTicketStatus(ticketId: string, newStatus: string) {
  ...
  try {
    ...
    // Record state change
    await recordStateChange(ticketId, ticket.estado, newStatus);
...
export async function reassignTicket(ticketId: string, newAgentId: string) {
  ...
  try {
    ...
    // Record assignment change
    await recordAssignmentChange(ticketId, currentTicket.asignado_a, newAgentId);
...
export async function updateTicketPriority(ticketId: string, newPriority: string) {
  ...
  try {
    ...
    await recordPriorityChange(ticketId, currentTicket.prioridad, newPriority);
